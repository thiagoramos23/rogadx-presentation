defmodule Concurrency.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      Concurrency.Repo,
      # Start the Telemetry supervisor
      ConcurrencyWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: Concurrency.PubSub},
      # Start the Endpoint (http/https)
      ConcurrencyWeb.Endpoint,
      # Start a worker by calling: Concurrency.Worker.start_link(arg)
      # {Concurrency.Worker, arg}
      {Task.Supervisor, name: Concurrency.CalculatorSupervisor},
      {DynamicSupervisor, strategy: :one_for_one, name: Concurrency.DynamicJobSupervisor},
      {Concurrency.CountWordSupervisor, name: Concurrency.CountWordSupervisor}
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: Concurrency.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    ConcurrencyWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
